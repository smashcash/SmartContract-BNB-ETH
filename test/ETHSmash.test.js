...
const Smash = artifacts.require('./ETHSmash.sol')
...
contract('ETHSmash', accounts => {
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const etherDenomination = await Smash.denomination()
  ...
  describe('#deposit', () => {
    it('should emit event', async () => {
      ...
      let { logs } = await Smash.deposit(commitment, { value, from: sender })
      ...
      ({ logs } = await Smash.deposit(commitment, { value, from: accounts[2] }))
    ...
    it('should throw if there is a such commitment', async () => {
      ...
      await Smash.deposit(commitment, { value, from: sender }).should.be.fulfilled
      const error = await Smash.deposit(commitment, { value, from: sender }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      // let gas = await Smash.deposit.estimateGas(toBN(deposit.commitment.toString()), { value, from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await Smash.deposit(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })
      ...
      const balanceSmashBefore = await web3.eth.getBalance(Smash.address)
      ...
      const { logs } = await Smash.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })

      const balanceSmashAfter = await web3.eth.getBalance(Smash.address)
      ...
      balanceSmashAfter.should.be.eq.BN(toBN(balanceSmashBefore).sub(toBN(value)))
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit()
      await tree.insert(deposit.commitment)
      await Smash.deposit(toFixedHex(deposit.commitment), { value, from: sender })
      ...
      await Smash.withdraw(proof, ...args, { from: relayer }).should.be.fulfilled
      const error = await Smash.withdraw(proof, ...args, { from: relayer }).should.be.rejected
    ...
    it('should reject with tampered public inputs', async () => {
      ...
      let error = await Smash.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      ...
      error = await Smash.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Invalid withdraw proof')
      ...
      await Smash.withdraw(proof, ...args, { from: relayer }).should.be.rejected

      // should work with original values
      await Smash.withdraw(originalProof, ...args, { from: relayer }).should.be.fulfilled
    })
  ...
  describe('#changeOperator', () => {
    it('should work', async () => {
      let operator = await Smash.operator()
      operator.should.be.equal(sender)

      const newOperator = accounts[7]
      await Smash.changeOperator(newOperator).should.be.fulfilled

      operator = await Smash.operator()
    ...
    it('cannot change from different address', async () => {
      ...
      const newOperator = accounts[7]
      const error = await Smash.changeOperator(newOperator, { from:  accounts[7] }).should.be.rejected
  ...
  describe('#updateVerifier', () => {
    it('should work', async () => {
      ...
      await Smash.updateVerifier(newVerifier).should.be.fulfilled

      const verifier = await Smash.verifier()
    ...
    it('cannot change from different address', async () => {
      ...
      const newVerifier = accounts[7]
      const error = await Smash.updateVerifier(newVerifier, { from:  accounts[7] }).should.be.rejected
      error.reason.should.be.equal('Only operator can call this function.')
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await Smash.deposit(toFixedHex(deposit1.commitment), { value, gasPrice: '0' })
      await Smash.deposit(toFixedHex(deposit2.commitment), { value, gasPrice: '0' })
      ...
      await Smash.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })
      ...
      const spentArray = await Smash.isSpentArray([nullifierHash1, nullifierHash2])
