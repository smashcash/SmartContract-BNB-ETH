...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  ...
  if (inBrowser) {
    ...
    contractJson = await (await fetch('build/contracts/ETHSmash.json')).json()
    ...
    contractJson = require('./build/contracts/ETHSmash.json')
    ...
    ETHSmashJson = require('./build/contracts/ETHSmash.json')
  }
